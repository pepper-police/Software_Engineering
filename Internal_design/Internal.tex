...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 予約語
    \begin{itemize}
      \item 予約後(true, false, null)は小文字で書く
      \item タイプキーワードは短い形式を利用する．(例：integer$\rightarrow$int)
    \end{itemize}
  \end{itemize}
  \item コーディングスタイル
  \begin{itemize}
    ...
    \item 行
    \begin{itemize}
      \item 1行の長さは80文字を超えてはならない．超える場合は複数の行に複数の行に分割する．
      \item 行末には空白を入れてはならない
      \item 1行に複数のステートメントがあってはならない
    \end{itemize}
    \item 文字列
    \item コメント
    \item クラス
    \begin{itemize}
      \item extendsとimplementsは、クラス名と同じ行で定義する
      \item クラスの開き括弧は次の行に記述する
      \item 閉じ括弧は本文最後の次の行に記述する
      \item implementsは複数の行に分割しても良い．その場合は，最初の定義も次の行から始め，１行に1つのインターフェイスを定義する
    \end{itemize}
    \item 演算子
  \end{itemize}
\end{itemize}
...
\section{クラスの依存関係}
\begin{itemize}
  \item Admin，Coop，UserクラスはAccountクラスを継承する．
  \item Account，DroneクラスはPairClassを継承する．
  \item PiarClassはInterface1インターフェイスを実装する．
  \item Admin，Coop，UserクラスはInterface2インターフェイスを実装する．
  \item DroneクラスはInterfaceDroneインターフェイスを実装する．
  \item Admin，CoopクラスはManagerクラスをメンバとして持つ．
  \item Coop，UserクラスはConsumerクラスをメンバとして持つ．
\end{itemize}
